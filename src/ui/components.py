...
class import_granulo_frame(ttk.Frame):
    """Composant pour l'importation de courbes granulométriques numériques ou réelles"""
    def __init__(self, parent, app, graphe, type_import, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        self.app, self.graphe, self.type = app, graphe, type_import
        # On force le fond de la frame d'import à blanc aussi pour être raccord avec la sidebar
    ...
    def _setup(self):
        ...
        ttk.Label(self, image=logo_ref, style="Sidebar.TLabel").grid(row=0, column=0, padx=5)
        ttk.Label(self, text=txt, style="Sidebar.TLabel").grid(row=0, column=1, sticky='w')
    ...
    def _import(self):
        if self.type == 'num':
            ...
            if path:
                ...
                self.app.my_granulos.num.granulo = {'x_axis': granulo["tamis"], 'y_axis': granulo["cumul"]}
                ...
                self.app.my_granulos.originale.granulo = {"x_axis": inv_x, "y_axis": granulo["cumul"]}
            ...
            if path:
                ...
                self.app.my_granulos.prat.granulo = {'x_axis': df.iloc[:,0].tolist(), 'y_axis': df.iloc[:,1].tolist()}
...
class correct_frame(ttk.Frame):
    """Composant global pour la correction des courbes numériques"""
    def __init__(self, parent, app, graphe, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        self.app, self.graphe = app, graphe
        self._build_ui()

    def _build_ui(self):
        ttk.Label(self, text="Correction Automatique", style="Title.TLabel").pack(pady=5)
        err_f = ttk.Frame(self)
        err_f.pack(fill="x")
        ttk.Label(err_f, text="Erreur :").pack(side="left")
        ttk.Label(err_f, textvariable=self.app.erreur).pack(side="left", padx=5)
        ttk.Button(err_f, text="Calculer Auto", command=self._auto).pack(side="right")

    def _auto(self):
        orig = self.app.my_granulos.originale.granulo
        prat = self.app.my_granulos.prat.granulo
        res = minimize(erreur_minim, [1.0, 0.0], args=(np.array(orig["x_axis"]), np.array(orig["y_axis"]), 
...
class barre_correct_frame_nv(ttk.Frame):
    ...
    def _build_ui(self):
        ttk.Label(self, text="Paramètres manuels", font=("Segoe UI", 10, "bold")).grid(row=0, column=0, columnspan=3, pady=(5,10))
    ...
    def _validate_change(self):
        try:
            ...
            self.app.my_granulos.num.granulo["x_axis"] = correct(orig["x_axis"], scale, offset)
...
class correct_frame(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Label(auto_f, textvariable=self.app.erreur, font=("Segoe UI", 10, "bold")).pack(side="left", padx=5)
        ...
        # Section Manuelle
        self.manual_f = barre_correct_frame_nv(self, self.app, self.graphe)
