...
en fonction des données fournies par l'état global de l'application.
    ...
    def _maj_cumuls(self):
        ...
        for info in dict_infos.values():
            if info["flag"] == 1 and info["courbe"] is not None:
                self.ax.plot(
                    info["courbe"]["x_axis"], info["courbe"]["y_axis"],
                      label=info["name"], color=info["color"], linewidth=2)
