...
class CIMESApp(tk.Tk):
    ...
    def _build_layout(self):
        # Sidebar
        sidebar = tk.Frame(self, width=350, bg=self.style_manager.BG_SIDEBAR, padx=15, pady=15)
        ...
        if self.logo_entreprise_side:
            lbl_logo = tk.Label(sidebar, image=self.logo_entreprise_side, bg=self.style_manager.BG_SIDEBAR)
            lbl_logo.pack(side="bottom", pady=(20, 0))
        # Components in Sidebar
        # Importation
        ttk.Label(
            sidebar, text="Importation", style="Sidebar.Title.TLabel").pack(pady=(0,10))
        ...
        # Gestion des courbes
        ttk.Label(
            sidebar, text="Gestion des Courbes", style="Sidebar.Title.TLabel").pack(pady=(0,10))
